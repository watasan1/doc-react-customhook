...
vite（ヴィート） は、Web アプリを開発するときに使う 開発サーバー兼ビルドツール です。

開発サーバー
→ 開発中のアプリをブラウザーに表示して確認できる仕組み。

ビルドツール
→ 開発中コードをブラウザで動く形に変換する
→ 複数のJSファイルやCSSファイルをひとつにまとめて効率的に読み込めるようにする（バンドラー）
→ 不要な部分の削除して軽くする
→ エラーを表示してくれる
→ ファイルを保存すると自動でブラウザに反映（HMR: Hot Module Replacement）

### 2. Viteの導入手順

ここからは、実際に vite を使って React プロジェクトを作ってみましょう。

1. 事前準備

まずは Node.js と npm が入っているか確認してください。
...
[https://nodejs.org/ja](https://nodejs.org/ja)からインストールしてください。 
